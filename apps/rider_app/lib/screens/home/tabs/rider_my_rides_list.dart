        ...
        if (rides.isEmpty)
          ...
          ...rides.map((dynamic raw) {
            ...
            return Padding(
              ...
              child: _RiderRideCard(api: api, ride: m, onUseRideId: onUseRideId),
            );
          }),
      ],
    );
...
class _RiderRideCard extends StatelessWidget {
  const _RiderRideCard({required this.api, required this.ride, required this.onUseRideId});
  ...
  static LatLng? _latLngFromGeo(Object? geo) {
    if (geo is! Map) return null;
    final Map<String, dynamic> m = Map<String, dynamic>.from(geo);
    final double? lat = (m['lat'] as num?)?.toDouble();
    final double? lng = (m['lng'] as num?)?.toDouble();
  ...
  Widget build(BuildContext context) {
    ...
    final LatLng? pickupLatLng = _latLngFromGeo(ride['pickup']);
    final LatLng? dropLatLng = _latLngFromGeo(ride['drop'] ?? ride['drop_location']);
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                if (pickupLatLng != null && dropLatLng != null) ...<Widget>[
                  const SizedBox(height: 10),
                  _RideRouteExpansion(api: api, pickup: pickupLatLng, drop: dropLatLng),
                ],
                if (otp != null) ...<Widget>[
...
class _RideRouteExpansion extends HookWidget {
  const _RideRouteExpansion({
    required this.api,
  ...
  Widget build(BuildContext context) {
    ...
    return ExpansionTile(
      ...
      onExpansionChanged: (bool open) => expanded.value = open,
      children: <Widget>[
        if (expanded.value)
          Padding(
            padding: const EdgeInsets.only(bottom: 8),
            child: RidePolylineMap(api: api, pickup: pickup, drop: drop, height: 200),
          ),
      ],
    );
  }
