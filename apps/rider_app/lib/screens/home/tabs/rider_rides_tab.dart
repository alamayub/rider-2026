...
import '../../../models/booking_payloads.dart';
import '../../../services/rider_api.dart';
import '../../../widgets/booking/booking_map_sheet.dart';
import '../../../widgets/home/console_widgets.dart';
import '../../../widgets/maps/ride_polyline_map.dart';
...
class _RiderCouponsBottomSheetBody extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextEditingController rideSearch = useTextEditingController();
    final TextEditingController couponSearch = useTextEditingController();
    useListenable(rideSearch);
    useListenable(couponSearch);
    final load = useState<Future<List<dynamic>>>(api.listAvailableCoupons());
    final selectedCode = useState<String?>(null);
    final busy = useState<bool>(false);

    void retryLoad() {
      load.value = api.listAvailableCoupons();
    }

    List<Map<String, dynamic>> filteredRides() {
      final String q = rideSearch.text.trim().toLowerCase();
    ...
    List<Map<String, dynamic>> filterCoupons(List<dynamic> list) {
      final String q = couponSearch.text.trim().toLowerCase();
      if (q.isEmpty) {
        return list
    ...
    final String rideIdTrim = rideIdController.text.trim();
    final List<Map<String, dynamic>> rideRows = filteredRides();
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Padding(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: rideSearch.text.isNotEmpty
                      ? IconButton(
            ...
            Padding(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: couponSearch.text.isNotEmpty
                      ? IconButton(
                          icon: const Icon(Icons.clear, size: 20),
                          onPressed: () {
                            couponSearch.clear();
            ...
            Padding(
              ...
              child: Text(
                fare > 0
                    ? 'Fare for validation: ${fare.toStringAsFixed(0)}'
                ...
                builder: (BuildContext ctx, AsyncSnapshot<List<dynamic>> snap) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (BuildContext ctx, int i) {
                      ...
                      final bool sel = selectedCode.value == code;
                      return Card(
            ...
            Padding(
              ...
              child: Wrap(
                ...
                children: <Widget>[
                  ElevatedButton(
                    onPressed: busy.value || selectedCode.value == null
                        ? null
                        : () async {
                            busy.value = true;
                            try {
                              await onValidateCoupon(selectedCode.value!);
                            } finally {
                              if (context.mounted) {
                                busy.value = false;
                              }
                  ...
                  OutlinedButton(
                    onPressed: busy.value ||
                        ...
                        : () async {
                            busy.value = true;
                            try {
                              await onApplyCoupon(selectedCode.value!);
                            } finally {
                              if (context.mounted) {
                                busy.value = false;
                              }
                  ...
                  OutlinedButton.icon(
                    onPressed:
                        busy.value || rideIdTrim.isEmpty ? null : () async {
                            busy.value = true;
                            try {
                              await onGetRideDetail();
                            } finally {
                              if (context.mounted) {
                                busy.value = false;
                              }
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Builder(
                    builder: (BuildContext ctx) {
                      final LatLng? ap = _latLngFromRideGeo(activeRide['pickup']);
                      final LatLng? ad =
                          _latLngFromRideGeo(activeRide['drop'] ?? activeRide['drop_location']);
                      if (ap == null || ad == null) return const SizedBox.shrink();
                      return Column(
                        crossAxisAlignment: CrossAxisAlignment.stretch,
                        children: <Widget>[
                          ...
                          ClipRRect(
                            ...
                            child: RidePolylineMap(api: api, pickup: ap, drop: ad, height: 200),
                          ),
                          Align(
                            alignment: Alignment.centerRight,
                            child: TextButton.icon(
                              onPressed: () => pushRiderRideRouteFullscreen(
                                context: ctx,
                                api: api,
                                pickup: ap,
                                drop: ad,
