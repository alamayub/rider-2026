...
class DriverSignInPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final phoneController = useTextEditingController(text: '9800000002');
    ...
    Future<void> requestOtp() async {
      ...
      message.value = null;
      try {
        await ref.read(driverApiProvider).requestOtp(phone: phoneController.text.trim());
        message.value = 'OTP request submitted.';
      } catch (e) {
        error.value = readError(e);
      } finally {
        loading.value = false;
    ...
    return Scaffold(
      appBar: AppBar(title: Text(isRegister.value ? 'Driver — Create account' : 'Driver — Sign in')),
      body: Center(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    TextButton(
                      ...
                      child: Text(
                        isRegister.value ? 'Already have an account? Sign in' : 'New driver? Create an account',
                      ),
                    ),
                    if (message.value != null) ...<Widget>[
                      const SizedBox(height: 8),
                      Text(message.value!, style: const TextStyle(color: Colors.green)),
                    ],
